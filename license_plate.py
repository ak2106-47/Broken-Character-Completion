...
fixed_z_ = noisy_images(train_set[140:165])
...
train_writer = tf.summary.FileWriter("./losses",sess.graph)
# results save folder
root = 'results/'
model = 'CHARS_'
...
for epoch in range(train_epoch):
    ...
    for iter in range(len(train_set) // batch_size):
        ...
        z_ = noisy_images(x_)
        summary,loss_d_, _ = sess.run([merge,D_loss, D_optim], {x: x_, z: z_, isTrain: True})
    ...
    saver.save(sess, './weights/weight',global_step=epoch,write_meta_graph=False)
